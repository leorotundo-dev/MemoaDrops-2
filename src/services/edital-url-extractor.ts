 ...
 */
async function extractEditalUrlFromPage(contestUrl: string, banca: string): Promise<string | null> {
  try {
    ...
    switch (banca.toLowerCase()) {
      ...
      case 'fundatec':
        ...
        editalUrl = $('a[href*="edital"]').first().attr('href') ||
                    ...
                    $('.documentos a[href$=".pdf"]').first().attr('href');
        break;
      ...
      case 'fcc':
        ...
        editalUrl = $('a:contains("Edital")').first().attr('href') ||
                    $('a[href*="edital"]').first().attr('href') ||
                    $('.downloads a[href$=".pdf"]').first().attr('href');
        break;
      ...
      case 'vunesp':
        ...
        editalUrl = $('a:contains("Edital")').first().attr('href') ||
                    $('a[href*="edital"]').first().attr('href') ||
                    $('.editais a[href$=".pdf"]').first().attr('href');
        break;
    ...
    console.error(`[Edital Extractor] Erro ao acessar ${contestUrl}:`, error.message);
    return null;
  }
}
...
export async function extractAllEditalUrls(): Promise<{
  ...
  try {
    ...
    const concursos = await db.raw(`
      SELECT id, name, contest_url, banca
      FROM concursos
    ...
    for (const concurso of concursos.rows) {
      ...
      try {
        const editalUrl = await extractEditalUrlFromPage(concurso.contest_url, concurso.banca);
        ...
        console.error(`[Edital Extractor] ❌ Erro:`, error.message);
      }
    ...
    console.error('[Edital Extractor] Erro fatal:', error);
    throw error;
  }
}
 ...
 */
export async function extractEditalUrlsByBanca(banca: string): Promise<{
  total: number;
  extracted: number;
  failed: number;
  results: EditalExtractionResult[];
  ...
  try {
    ...
    result.total = concursos.rows.length;
    console.log(`[Edital Extractor] Encontrados ${result.total} concursos da banca ${banca}`);
